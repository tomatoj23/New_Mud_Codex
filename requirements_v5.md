...
- 根目录 `UBIQUITOUS_LANGUAGE.md` 为当前现行术语表，用于对齐本版本第八章与下游设计文档中的统一命名；若与本版本发生冲突，以本版本为准。
...
- 术语边界以本版本第八章为主，`UBIQUITOUS_LANGUAGE.md` 负责跨文档统一表达。
...
具体部署者自行承担目标环境中的内容使用与发布责任。工程文档不得用 `content_release_mode` 或同类许可状态阻断实现、测试、里程碑或发布候选。
...
首发样板区域固定为 `xkx100-village-alley-v1`，来源为 `d/village` 的小巷纵切。
...
当前术语权威来源为本章与根目录 `UBIQUITOUS_LANGUAGE.md`；若两者表述粒度不同或发生冲突，以本章为准。

以下术语定义即构成本版本的当前权威术语集；后续若需重建新的术语表，必须以本章与本版本其他已冻结约束为基线：
...
首发必须提供用户名密码自助注册。注册标识使用独立账号名，不要求手机号或邮箱。
...
M1 必须提供受审计的后台密码重置和账号恢复最小流程。无需手机号或邮箱的玩家自助找回仍属于 M2。
...
M0 必须批准并版本控制首发 `capacity_profile`。在没有更高目标的已批准 profile 时，M1 使用以下最低基线：
...
延迟从服务端收到完整请求到产生可交付终结或事件计算；不包含客户端公网传输和显式 Scheduler 等待。测试报告必须记录硬件、数据集、采样窗口和 P50/P95/P99。
...
| M1-B 发布候选 | 白名单后台编辑、发布/回滚、调度恢复、浏览器矩阵、容量、安全和备份恢复全部通过 | 等同 M1 完成 |

M1-A 不是独立需求里程碑。只有 M1-B 通过，需求里程碑 M1 才能标记 `complete`。
...
- M0 必须批准 capacity profile、精确浏览器矩阵与恢复预算
...
- `docs/00-19` 中涉及现行术语、权威顺序与路线图编号的分析或治理说明
- 根目录 `UBIQUITOUS_LANGUAGE.md`

同步重点包括注册与认证、控角约束、ConditionDefinition / EffectInstance、固定夹具、兼容包络、发布闭环、来源可复现性、容量恢复预算、需求追溯和小程序交付。

后续修改产品目标、范围、里程碑或验收结果时，必须先修改 V5，并在同一变更中同步受影响合同和追踪索引。

只改变协议字段、状态机、持久结构、事务或失败语义且不改变产品结果时，应修改对应冻结合同和追踪索引；V5 只在跨系统不变量或验收结果变化时同步。

冲突必须先按第 1.4 节确定关注点。产品问题以 V5 为准，实施问题以对应冻结合同为准；无法分类时阻断合并，不能用简单的文件总排序掩盖问题。

---

## 十八、需求标识与追踪

`docs/new_engine/17_REQUIREMENTS_TRACEABILITY.md` 维护稳定需求 ID、V5 来源、实施合同、里程碑和验收证据。
