...
- 根目录 `CONTEXT.md` 是项目特有领域词汇表；`UBIQUITOUS_LANGUAGE.md` 保留更细的工程术语。若两者或下游文档与本版本发生产品语义冲突，以本版本为准。
...
当前术语权威来源为根目录 `CONTEXT.md` 与本章；`CONTEXT.md` 冻结项目领域边界，本章冻结身份产品语义。两者粒度不同或产品语义发生冲突时，以本章及 V6 其他已冻结约束为准。`UBIQUITOUS_LANGUAGE.md` 是工程术语伴随文档，只负责跨文档统一表达，不建立第三份权威。
