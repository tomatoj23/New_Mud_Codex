...
当前仓库尚无 New_Mud 实现。除已明确废弃的条目外，以下条目统一为 `specified`；代码、迁移、测试和审批出现后，必须填写可定位的证据路径或 CI 制品 ID。
...
| `CONTENT-001` | `specified` | 内容通过不可变 revision、完整批次、冷发布和批次回滚生效 | 5.4、6.13-6.14 | 12 | M0、M1 | 原子发布、依赖重编译、回滚测试 |
| `ADMIN-001` | `specified` | M1 后台只编辑白名单对象，并实施角色分权和自批禁止 | 12.1-12.4 | 08 第 1-3 节、12 | M1 | 权限矩阵、审计、发布 E2E |
| `CLIENT-001` | `specified` | PC 与移动 H5 在固定浏览器、视口、中文输入和无障碍矩阵通过 | 9.2.1、9.3-9.5 | 15 | M1 | 精确版本矩阵、视觉和交互 E2E |
| `NFR-001` | `specified` | 默认 capacity profile 的负载、延迟和稳定运行目标全部达标 | 13.4-13.7 | 16 第 3/7 节 | M0、M1 | 容量报告、指标与两小时 soak |
| `NFR-002` | `specified` | 备份保留、RPO、RTO 和隔离恢复演练全部达标 | 13.2、14.4、13.7 | 16 第 5-6 节 | M0、M1 | 备份清单、恢复演练与审批 |
| `MILESTONE-001` | `specified` | M0 制品、合同、发布契约和非功能 profile 全部冻结 | 15.0-15.1 | 10 E0、16 | M0 | M0 门禁清单及自动校验 |
| `MILESTONE-002` | `specified` | M1-A 仅为内部可玩验证，M1-B 才等同 M1 完成 | 15.0、15.2 | 10 E1-E9、16 第 10 节 | M1 | 两阶段门禁与发布候选报告 |
| `CONVERT-001` | `specified` | 转换和黄金验收绑定不可变 source snapshot、双 manifest 与 bundle | 7.2-7.16 | 09、16 第 8 节 | M0、M4 | 快照、manifest、bundle 与差分制品 |
