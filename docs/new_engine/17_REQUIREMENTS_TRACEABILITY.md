...
| `CLIENT-001` | `blocked` | PC 与移动 H5 在固定浏览器、视口、中文输入和无障碍矩阵通过 | 9.2.1、9.3-9.5 | 15 | M1 | `contracts/v1/profiles/browser-matrix.json` 已建；精确版本、审批、视觉和交互 E2E 缺失 |
| `NFR-001` | `blocked` | 默认 capacity profile 的负载、延迟和稳定运行目标全部达标 | 13.4-13.7 | 16 第 3/7 节 | M0、M1 | `contracts/v1/profiles/capacity-profile.json` 已建；审批、容量报告与两小时 soak 缺失 |
| `NFR-002` | `blocked` | 备份保留、RPO、RTO 和隔离恢复演练全部达标 | 13.2、14.4、13.7 | 16 第 5-6 节 | M0、M1 | `contracts/v1/profiles/recovery-budget.json` 已建；审批与隔离恢复报告缺失 |
| `MILESTONE-001` | `blocked` | M0 制品、合同、发布契约和非功能 profile 全部冻结 | 15.0-15.1 | 10 E0、16 | M0 | `scripts/verify_m0.py` 与 `.github/workflows/m0.yml`；结构通过，非功能审批和报告阻断 |
