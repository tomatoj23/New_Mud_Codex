...
| E1 / Auth Baseline Amendment RecoveryCode 退役（2026-08-27，Issue #15） | `RUN_POSTGRES_TESTS=1` 串行全量 234 passed；认证专项 63 passed，生产启动/配置专项 8 passed；覆盖两个旧端点统一 410、旧 code 不可消费、迁移撤销 active code、数据库拒绝新增 active、回滚不复活秘密，以及 cutover/keyring/worker/provider 故障统一 fail closed 且普通登录可用。Vue typecheck、Vitest 12 passed、H5 build 与 Playwright 13 passed/8 skipped 通过；Ruff lint、90 files format、mypy 90 source files、Django 0 issues、无 migration drift、`pip check` 与 npm critical audit 通过；`verify_m0.py` 57,156 checks、READY |
