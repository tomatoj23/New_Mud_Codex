...
`PRESENCE_RECOVERY_UNAVAILABLE`、`RECOVERY_CODE_RETIRED`、
`ACCOUNT_REOPEN_WINDOW_EXPIRED`、`ACCOUNT_NOT_REOPENABLE`、`ACCOUNT_ALREADY_RETIRED`。

已退役的实现 provenance：`RECOVERY_CODE_INVALID`、`RECOVERY_RATE_LIMITED`、`ACCOUNT_RECOVERY_UNAVAILABLE` 只标记 Issue #9 与 Issue #15 原子切换前的历史实现/测试证据，不属于现行稳定错误集合，新实现不得返回这些错误。
